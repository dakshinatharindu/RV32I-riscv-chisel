...
for instr in ReadFile:
    ...
    if(ty[0] == "R"):
        ...
        code = imm[0:7] + bin(int(rs2))[2:].rjust(5, "0") + \
            bin(int(rs1))[2:].rjust(5, "0") + ty[1] + imm[7:] + op

    elif(ty[0] == "B"):
        rs1 = instr[1]
        rs2 = instr[2]
        imm = bin(((1 << 13) - 1) & int(instr[3]))[2:].rjust(13, "0")
        code = imm[0] + imm[2:8] + bin(int(rs2))[2:].rjust(5, "0") + bin(
            int(rs1))[2:].rjust(5, "0") + ty[1] + imm[8:12] + imm[1] + op
        ...
        code = imm[0] + imm[10:20] + imm[9]+imm[1:9] + \
            bin(int(rd))[2:].rjust(5, "0") + op
    else:
        assert False, "Invalid Instruction!!!"

    WriteFile.write(hex(lineNumber)[2:] + " : " + code + ";\n")
    lineNumber += 1
WriteFile.write("[{}..{}] : 0;\n".format(hex(lineNumber)[2:], "1FF"))
WriteFile.write("END;\n")
WriteFile.close()
ReadFile.close()
